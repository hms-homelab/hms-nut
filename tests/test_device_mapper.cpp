...
TEST_F(DeviceMapperTest, LoadFromConfigsReplacesState) {
    setenv("UPS_DEVICE_IDS", "old_ups", 1);
    DeviceMapper::initialize();
    ASSERT_TRUE(DeviceMapper::isKnownDevice("old_ups"));

    std::vector<DeviceConfig> cfgs;
    DeviceConfig a; a.mqtt_device_id = "apc_a"; a.db_identifier = "apc_a_db"; a.friendly_name = "APC A";
    DeviceConfig b; b.mqtt_device_id = "apc_b"; b.db_identifier = "apc_b_db"; b.friendly_name = "";
    cfgs.push_back(a);
    cfgs.push_back(b);
    DeviceMapper::loadFromConfigs(cfgs);

    // Old (env-based) device is replaced by the DB config set
    EXPECT_FALSE(DeviceMapper::isKnownDevice("old_ups"));
    ASSERT_EQ(DeviceMapper::getDeviceIds().size(), 2u);
    EXPECT_EQ(DeviceMapper::getDbIdentifier("apc_a"), "apc_a_db");
    EXPECT_EQ(DeviceMapper::getMqttDeviceId("apc_b_db"), "apc_b");
    EXPECT_EQ(DeviceMapper::getFriendlyName("apc_a"), "APC A");
    // Empty friendly name falls back to generated form
    EXPECT_EQ(DeviceMapper::getFriendlyName("apc_b"), "Apc b");
}

TEST_F(DeviceMapperTest, LoadFromConfigsEmptyClears) {
    setenv("UPS_DEVICE_IDS", "x1,x2", 1);
    DeviceMapper::initialize();
    DeviceMapper::loadFromConfigs({});
    EXPECT_TRUE(DeviceMapper::getDeviceIds().empty());
    EXPECT_FALSE(DeviceMapper::isKnownDevice("x1"));
}

TEST_F(DeviceMapperTest, LoadFromConfigsEmptyDbIdDefaultsToMqttId) {
    std::vector<DeviceConfig> cfgs;
    DeviceConfig c; c.mqtt_device_id = "solo"; c.db_identifier = ""; c.friendly_name = "";
    cfgs.push_back(c);
    DeviceMapper::loadFromConfigs(cfgs);
    EXPECT_EQ(DeviceMapper::getDbIdentifier("solo"), "solo");
}
