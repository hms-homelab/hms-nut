...
public:
    ...
    /**
     * Reconcile MQTT subscriptions against the current DeviceMapper device list.
     * Subscribes to newly-added devices and unsubscribes removed ones, without a
     * service restart. Call after the device_config is edited via the web UI.
     */
    void reloadSubscriptions();

    /**
     * Thread-safe copy of the current in-memory live data, keyed by
     * device_identifier. Used by the web UI's live status endpoint.
     */
    std::map<std::string, UpsData> snapshotData() const;
...
private:
    ...
    // Set of device IDs we currently hold MQTT subscriptions for (hot-reload)
    std::set<std::string> subscribed_device_ids_;
