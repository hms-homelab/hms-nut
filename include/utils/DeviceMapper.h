...
public:
    ...
    /**
     * Replace all mappings from a persisted config set (device_config table).
     * Used at startup (DB is authoritative) and on every hot-reload after an
     * edit via the web UI. Only pass enabled devices.
     *
     * @param configs Device configurations to load
     */
    static void loadFromConfigs(const std::vector<DeviceConfig>& configs);
