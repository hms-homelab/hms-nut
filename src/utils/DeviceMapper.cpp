...
void DeviceMapper::loadFromConfigs(const std::vector<DeviceConfig>& configs) {
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    for (const auto& c : configs) {
        if (c.mqtt_device_id.empty()) {
            continue;
        }
        if (std::find(device_ids_.begin(), device_ids_.end(), c.mqtt_device_id) == device_ids_.end()) {
            device_ids_.push_back(c.mqtt_device_id);
        }
        std::string db_id = c.db_identifier.empty() ? c.mqtt_device_id : c.db_identifier;
        mqtt_to_db_map_[c.mqtt_device_id] = db_id;
        db_to_mqtt_map_[db_id] = c.mqtt_device_id;
        if (!c.friendly_name.empty()) {
            friendly_names_[c.mqtt_device_id] = c.friendly_name;
        }
    }

    initialized_ = true;
    std::cout << "📋 DeviceMapper: loaded " << device_ids_.size()
              << " device(s) from DB config" << std::endl;
}
