...
void CollectorService::setupSubscriptions() {
...

    {
        ...
        for (const auto& device_id : device_ids) {
            ...
            subscribed_device_ids_.insert(device_id);
...
void CollectorService::reloadSubscriptions() {
    std::vector<std::string> desired_vec = DeviceMapper::getDeviceIds();
    std::set<std::string> desired(desired_vec.begin(), desired_vec.end());
    ...
    // Subscribe to newly-added devices
    for (const auto& id : desired) {
        if (subscribed_device_ids_.count(id) == 0) {
            std::string topic = "homeassistant/sensor/" + id + "/+/state";
            if (mqtt_client_->subscribe(topic, callback, 1)) {
                subscribed_device_ids_.insert(id);
                std::cout << "➕ Collector: subscribed to new device " << id << std::endl;
            } else {
                std::cerr << "⚠️  Collector: failed to subscribe " << topic << std::endl;
            }
        }
    }

    // Unsubscribe from removed/disabled devices
    for (auto it = subscribed_device_ids_.begin(); it != subscribed_device_ids_.end();) {
        if (desired.count(*it) == 0) {
            std::string topic = "homeassistant/sensor/" + *it + "/+/state";
            mqtt_client_->unsubscribe(topic);
            std::cout << "➖ Collector: unsubscribed removed device " << *it << std::endl;
            it = subscribed_device_ids_.erase(it);
        } else {
            ++it;
        }
    }
...
std::map<std::string, UpsData> CollectorService::snapshotData() const {
...
void CollectorService::onMqttMessage(const std::string& topic, const std::string& payload) {
    ...
    // Ignore messages from devices that are not currently configured/enabled
    // (guards against retained or in-flight messages after a device is removed).
    if (!DeviceMapper::isKnownDevice(mqtt_device_id)) {
        return;
    }
